...
{
    public partial class wucIncidenciaResumenSupervisor : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var amarilla = 0;
            var rojas = 0;
            var dt = new IncidenciaBC().GetDatosResumen(Convert.ToString(Session["usrname"]), Constantes.SP_RESUMEN_SUPERVISOR);
            if (dt.Rows.Count > 0)
            {
                ...
                string tipoResumen = Convert.ToString((int)Enums.TipoVistaResumen.Supervisor);

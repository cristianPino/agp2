...
{
	public partial class Ingreso_garantia_portillo : System.Web.UI.Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			if (!IsPostBack)
			{
				...
				FuncionGlobal.combobanco(this.dl_banco,Convert.ToInt32(ViewState["id_cliente"].ToString()));
		...
		protected bool Validar_Form()
		{
			...
			if (this.dl_forma_pago_factura.SelectedValue == "3")
			{
				...
				if (!int.TryParse(this.txt_monto_factura.Text, out total)) total = 0;
				if (!int.TryParse(this.txt_pie.Text, out pie)) pie = 0;
				if (!int.TryParse(this.txt_monto_factura_intereses.Text, out intereses)) intereses = 0;
		...
		//{
		//    if (this.txt_pie.Text != "")
		//    {
		//        int valor = Convert.ToInt32(this.txt_pie.Text.Replace(ViewState["separadorMiles"].ToString(), ""));
		//        this.txt_pie.Text = valor.ToString("N0");
		//    }
		//    this.Calcular_Monto_Financiar();
		//}
